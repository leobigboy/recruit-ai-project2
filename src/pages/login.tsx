...
// Import Supabase client từ thư mục lib
...
// -----------------------------------------------------------------------------
const AlertMessage = ({ message, type = "error", onClose }: { message: string | null; type?: "error" | "success"; onClose: () => void; }) => {
    if (!message) return null;
    const baseStyles = "p-4 rounded-xl shadow-lg flex items-start justify-between mb-6 border";
    const variantStyles = { error: "bg-red-900/40 text-red-300 border-red-700", success: "bg-green-900/40 text-green-300 border-green-700" };
...
const Button = ({ children, variant = "default", className = "", ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & { variant?: "default" | "outline" | "ghost" }) => {
    const baseStyles = "px-4 py-2 rounded-lg font-medium transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed shadow-md hover:shadow-lg active:shadow-sm";
    const variants = { default: "bg-white text-black hover:bg-gray-100", outline: "border border-gray-700 text-white hover:bg-gray-900 bg-black/10", ghost: "text-gray-400 hover:text-white hover:bg-gray-900" };
    return <button className={`${baseStyles} ${variants[variant]} ${className}`} {...props}>{children}</button>;
};

const Input = ({ className = "", ...props }: React.InputHTMLAttributes<HTMLInputElement>) => {
    return <input className={`w-full px-4 py-3 bg-black border border-gray-800 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-gray-600 transition-colors ${className}`} {...props} />;
};

const Label = ({ children, className = "", ...props }: React.LabelHTMLAttributes<HTMLLabelElement>) => {
    return <label className={`block text-sm font-medium text-gray-300 mb-2 ${className}`} {...props}>{children}</label>;
};
...
export default function LoginPage() {
    ...
    // Xử lý đăng nhập bằng Email/Password với Supabase
    ...
    // Xử lý đăng nhập mạng xã hội với Supabase
            ...
            <div className="w-full max-w-md">
                ...
                <div className="bg-black border border-gray-800 rounded-2xl p-8 shadow-2xl">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div>
                            <div className="flex items-center justify-between mb-2">
                                ...
                                <a href="/forgot-password" className="text-sm text-gray-400 hover:text-white transition-colors">Forgot password?</a>
                            </div>
                    ...
                    <div className="relative my-6">
                        ...
                        <div className="relative flex justify-center text-sm"><span className="px-4 bg-black text-gray-500">Hoặc tiếp tục với</span></div>
                    </div>

                    <div className="space-y-3">
                        <Button type="button" variant="outline" className="w-full flex items-center justify-center gap-3 bg-transparent text-gray-200 border-gray-700 hover:bg-gray-800" onClick={() => handleSocialLogin('google')} disabled={isLoading}>
                            <IconGoogle className="w-5 h-5" />
                            Continue with Google
                        </Button>
                        <Button type="button" variant="outline" className="w-full flex items-center justify-center gap-3 bg-transparent bg-blue-700/20 border-blue-700 text-white hover:bg-blue-700/30" onClick={() => handleSocialLogin('facebook')} disabled={isLoading}>
